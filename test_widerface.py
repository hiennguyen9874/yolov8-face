...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default="runs/pose/yolov8n-face/weights/best.pt",
        help="model.pt path(s)",
    )
    parser.add_argument(
        "--img-size", nargs="+", type=int, default=640, help="inference size (pixels)"
    )
    parser.add_argument(
        "--conf-thres", type=float, default=0.01, help="object confidence threshold"
    )
    ...
    parser.add_argument(
        "--dataset_folder", default="./data/widerface/val/images/", type=str, help="dataset path"
    )
    ...
    for img_name in test_dataset:
        ...
        with open(save_name, "w") as fd:
            ...
            for box in result.boxes:
                ...
                y2 = int(xyxy[3] + 0.5)
                fd.write(
                    "%d %d %d %d %.03f" % (x1, y1, x2 - x1, y2 - y1, conf if conf <= 1 else 1)
                    + "\n"
                )
