...
class DetectionModel(BaseModel):
    ...
    def __init__(
        ...
        if isinstance(m, (Detect, Segment, Pose)):
            ...
            forward = (
                lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose)) else self.forward(x)
            )
...
class ClassificationModel(BaseModel):
    ...
    def __init__(
        ...
        super().__init__()
        self._from_detection_model(model, nc, cutoff) if model is not None else self._from_yaml(
            cfg, ch, nc, verbose
        )
